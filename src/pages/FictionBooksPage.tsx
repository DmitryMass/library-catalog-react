...
import { FictionBookItem } from '@components/fictions/FictionBookItem';
...
const FictionBooksPage: FC = () => {
  ...
  const { data, isError, error, isLoading } = useQuery<TBooksResponse>({
    queryKey: [queryKey.fictionBooks, currentPage],
    queryFn: () => getBooks(BookTypeObject.fiction, currentPage),
  ...
  return (
    <div className="container w-full flex-1 py-10">
      <SearchPanel type={BookTypeObject.fiction} eachBook={PATH.fiction} />
      <>
        <div className="pb-14">
          {data?.books.length ? (
            <div>
              ...
              <DataList
                data={data.books}
                item={(item) => <FictionBookItem key={item._id} book={item} />}
              />
              {pageCount > 1 ? (
                <div className="mt-14">
                  <Pagination
