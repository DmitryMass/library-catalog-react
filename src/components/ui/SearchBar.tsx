...
export const SearchPanel: FC<TSearchPanel> = ({ type, eachBook }) => {
    ...
    const searchHandler = async () => {
      if (debauncedString.trim()) {
        if (!foundedBooksCache[debauncedString]) {
          try {
            const res = await $api.get(`catalog/search/${type}`, {
  ...
  return (
    <div className="mx-auto mb-14 flex max-w-xl flex-col-reverse items-center justify-center">
      <div
        className={clsx(
